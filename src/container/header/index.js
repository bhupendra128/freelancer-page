...
const Header = () => {
  ...
  useEffect(() => {
    ...
    bodyEle.classList.add("sticky");
    return () => {
      if (bodyEle.classList.contains("sticky")) {
        bodyEle.classList.remove("sticky");
      }
    };
  });
  const handleSelectedFilter = (arr) => {
    ...
    const updatedListingData = initialData.filter(
      (itm) => itm.avg_ratings >= minimumRating
    );
